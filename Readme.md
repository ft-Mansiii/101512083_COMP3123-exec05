...
tells Express to start the server on a port defined by the environment variable process.env.port. If that variable is not set, it defaults to port 8081.
n production, hosting platforms assign a port dynamically using an environment variable. This line allows your app to work both locally and in production (using the assigned port)
---
